...
df = df[df['C21B13'] != 0]
df["gpa_norm"] = df["C21B12"] / df["C21B13"]
...
plt.title("GPA Distribution (2021)")
...
for col in binary_vars:
    ...
    if len(yes) > 1 and len(no) > 1:
        ...
        plt.title(f"GPA vs {labels[col]} (2021)")
...
for col in ordinal_vars:
    ...
    plt.title(f"Distribution of {labels.get(col, col)} (2021)")
...
plt.title("Point-Biserial Correlations with GPA (2021)")
...
plt.title("Actual vs Predicted GPA (2021)")

...
df = df[df['C14B13'] != 0]
df["gpa_norm"] = df["C14B12"] / df["C14B13"]
...
plt.title("GPA Distribution (2014)")
...
for col in binary_vars:
    ...
    if len(yes) > 1 and len(no) > 1:
        ...
        plt.title(f"GPA vs {labels[col]} (2014)")
...
for col in ordinal_vars:
    ...
    if col == "electronics_music":
        bins = [0.5,1.5,2.5,3.5,4.5,5.5]  # 5
    elif col == "instrument_frequency":
        bins = [0.5,1.5,2.5,3.5,4.5,5.5,6.5]  # 6
    ...
    plt.title(f"Distribution of {labels.get(col, col)} (2014)")
...
plt.title("Point-Biserial Correlations with GPA (2014)")
...
plt.title("Actual vs Predicted GPA (2014)")
